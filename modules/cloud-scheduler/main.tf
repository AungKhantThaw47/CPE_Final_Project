...
locals {
  ...
  codebase_files = data.external.git_status.result.has_changes == "true" ? fileset(local.codebase_directory, "**") : []
  codebase_files_filtered = [
    for file in local.codebase_files :
    file if !startswith(file, "utils/") && !startswith(file, ".build-hash") && fileexists("${local.codebase_directory}/${file}")
  ]
